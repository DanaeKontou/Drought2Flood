...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (province) {
      ...
      // Query for province aggregates (one per province)
      const result = await client.query(`
        SELECT 
          province_code,
          COUNT(DISTINCT EXTRACT(YEAR FROM dstart)) as year_count,
          MIN(EXTRACT(YEAR FROM dstart))::integer as first_year,
          MAX(EXTRACT(YEAR FROM dstart))::integer as last_year,
          ST_AsGeoJSON(ST_Centroid(ST_Collect(geom)))::json as centroid,
